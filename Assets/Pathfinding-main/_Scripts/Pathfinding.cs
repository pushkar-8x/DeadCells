...
namespace Tarodev_Pathfinding._Scripts
{
    ...
    public static class Pathfinding
    {
        ...
        public static (List<NodeBase> path, NodeBase closestTarget) FindShortestPath(NodeBase startNode,
            List<NodeBase> targetNodes)
        {
            List<NodeBase> shortestPath = null;
            NodeBase closestTarget = null;
            float shortestDistance = float.MaxValue;

            foreach (var targetNode in targetNodes)
            {
                var path = FindPathToTarget(startNode, targetNode);
                if (path != null)
                {
                    float distance = path.Sum(node => node.G);
            ...
            return (shortestPath, closestTarget);
        }

        private static List<NodeBase> FindPathToTarget(NodeBase startNode, NodeBase targetNode)
        {
            ...
            while (toSearch.Any())
            {
                ...
                if (current == targetNode)
                {
                    ...
                    while (currentPathTile != startNode)
                    {
                        ...
                        count--;
                        if (count < 0) throw new Exception("Pathfinding error: infinite loop detected.");
                    }
